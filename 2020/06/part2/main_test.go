...
func TestNewGroup(t *testing.T) {
	cases := []struct {
		Desc, Input                          string
		MemberCnt, GroupYayCnt, ConsensusCnt int
	}{
		{
			Desc:         "1 member, 1 yay",
			Input:        "a",
		...
		{
			Desc:         "4 members, 4 yays, 2 consensus",
			Input:        "ab\nabc\nabz\nab",
			MemberCnt:    4,
			GroupYayCnt:  4,
			ConsensusCnt: 2,
	...
	for _, tc := range cases {
		t.Run(tc.Desc, func(t *testing.T) {
			...
			if tc.ConsensusCnt != actual.ConsensusCnt {
				t.Errorf("Expected %v group consensus, Got %v", tc.ConsensusCnt, actual.ConsensusCnt)
			}
		})
	}
}
